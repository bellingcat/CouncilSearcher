...
class Provider:
    ...
    @staticmethod
    def create(provider_name:str, db_path:str, authority:str, config:dict | None = None) -> 'Provider':
        """
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for uid, item in directory.items():
                conn.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                ''', (uid, self.authority, item['title'], item['description'], item['datetime'], item['unixtime'], item['link']))
...
class PublicI(Provider):
    ...
    def get_meetings(self) -> list[MeetingItem]:
        ...
        with ThreadPoolExecutor() as executor:
            results = list(tqdm(executor.map(self._parse_index_item, self.index), total=len(self.index)))
    ...
    def _resolve_urls(self) -> tuple[str, str]:
        ...
        if authority_id == None:
            raise Exception("Could not find ds_id input or value in the magic RSS page.")
    ...
    def _parse_index_item(self, full_item: dict) -> MeetingItem:
        ...
        unix_time = int(datetime.strptime(date, "%a, %d %b %Y %H:%M:%S %z").timestamp()) if date else None
        date_time = datetime.strptime(date, "%a, %d %b %Y %H:%M:%S %z").isoformat(" ") if date else None
